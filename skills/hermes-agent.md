...
- **WeChat / Weixin setup:** `references/weixin-wechat-setup.md` — QR login flow, env vars, iLink Bot limitations (no group chat), troubleshooting
...
| **Cost optimization** | `references/token-cost-optimization.md` — system prompt bloat (8.5K/turn) is the real cost driver, not model choice. Insight: lightweight profile + script-relay to cheap API saves more than swapping default model.
...
- **WeChat / Weixin setup**: See `references/weixin-wechat-setup.md` for QR login, env vars, and iLink Bot limits.
- **Driving `hermes gateway setup` from within Hermes**: See `references/driving-gateway-setup-from-agent.md` for the curses navigation pattern when automating setup via the process tool.
...
| Dual API / model routing | `references/dual-api-routing.md` (双API双保险方案 + 系统提示词开销分析 + 轻量Profile方案 + 本地模型可行性评估) |
| Cost optimization | `references/dual-api-routing.md` — 四种方案（轻量Profile/Script Relay/本地模型/多API路由）详见**真正的成本瓶颈：系统提示词开销**节 |
